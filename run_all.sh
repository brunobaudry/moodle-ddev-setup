  ...
  for php in "${PHP_VERSIONS[@]}"; do
    ...
    if validate_compatibility "$moodle" "$php"; then
      for db in "${DB_TYPES[@]}"; do
        echo "🏃🏻‍➡️ Running: ./moodle_ddev.sh --php $php --version $moodle --db $db --repo composer --root $root_folder --admincfg-csv $csv_admin_cfg"
        if $force; then 
          ./moodle_ddev.sh --php "$php" --version "$moodle" --db "$db" --repo "composer" --force --root "$root_folder" --admincfg-csv $csv_admin_cfg
          ...
          ./moodle_ddev.sh --php "$php" --version "$moodle" --db "$db" --repo "composer" --root "$root_folder" --admincfg-csv $csv_admin_cfg
...
./sub/ddev_describe_all.sh --root "$root_folder"